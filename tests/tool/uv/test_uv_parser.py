...
@pytest.mark.unit
class TestUvParser:
    """Test UV-specific parser functionality"""
    ...
    def test_uv_parser_tool_install_command(self):
        """Test uv tool install command"""
        intention = self.parser.parse(["tool", "install", "black"])

        assert intention is not None
        assert intention.command == "tool install"
        assert intention.command_chain == ["tool", "install"]
        assert intention.intention_type == ToolIntentionType.ADD_PACKAGE
        assert len(intention.packages) == 1
        assert intention.packages[0].name == "black"

    def test_uv_parser_tool_uninstall_command(self):
        """Test uv tool uninstall command"""
        intention = self.parser.parse(["tool", "uninstall", "black"])

        assert intention is not None
        assert intention.command == "tool uninstall"
        assert intention.command_chain == ["tool", "uninstall"]
        assert intention.intention_type == ToolIntentionType.REMOVE_PACKAGE
        assert len(intention.packages) == 1
        assert intention.packages[0].name == "black"
    ...
    def test_uv_parser_tool_install_flags(self):
        """Test UV tool install flags"""
        intention = self.parser.parse(
            ["tool", "install", "--editable", "--force", "black"]
        )

        assert intention is not None
        assert intention.command_chain == ["tool", "install"]

        # Tool install flags should be recognized
        assert "editable" in intention.options
        assert intention.options["editable"]["value"] is True
        assert "force" in intention.options
        assert intention.options["force"]["value"] is True

        # Package should still be parsed
        assert len(intention.packages) == 1
        assert intention.packages[0].name == "black"

    def test_uv_parser_short_flags(self):
        """Test UV short flag variants"""
        intention = self.parser.parse(["tool", "install", "-e", "black"])

        assert intention is not None

        # Short flag should be recognized
        assert "e" in intention.options
        assert intention.options["e"]["value"] is True
