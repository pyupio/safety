...
from safety.init.main import PROJECT_CONFIG_ID, PROJECT_CONFIG_NAME, \
    PROJECT_CONFIG_URL, PROJECT_CONFIG_SECTION, check_project, create_project, save_project_info, \
...
class TestInitMain(unittest.TestCase):
    ...
    def test_check_project_without_id(self, mock_wait_verification,
                                      mock_prompt_id):
        """
        ...
        unverified_project = Mock(
            id=None,
            project_path=Path("/test/dir/project/.safety-project.ini")
        )
        ...
        _ = check_project(ctx, session, console, unverified_project,
                               git_origin=None)
    ...
    def test_save_project_info_success(self, mock_config_cls):
        ...
        for case_name, project, expected_config in test_cases:
            with self.subTest(case_name=case_name):
                ...
                result = save_project_info(project, "test_config.ini")
                ...
                calls = [
                    call(key, value) for key, value in expected_config.items()]
                section_mock.__setitem__.assert_has_calls(calls,
                                                          any_order=False)
    ...
    def test_create_project_with_platform_enabled(self, mock_verify, mock_git, \
                                                  mock_load_project):
        ctx = Mock()
    ...
    @patch('safety.init.main.GIT')
    @patch('safety.init.main.verify_project')
    def test_create_project_platform_disabled(self, mock_verify, mock_git, mock_load_project):
        ctx = Mock()
