...
def format_vulnerability(
    ...
    if cve:
        ...
        if cve.cvssv3 and "base_severity" in cve.cvssv3.keys():
            cvss_base_severity_style: dict[str, str | bool] = {"bold": True}
    ...
    if is_using_api_key():
        fixed_version_line = {
            "words": [
                ...
                {
                    "value": ", ".join(vulnerability.fixed_versions)
        ...
        more_info_line.insert(
            0,
            {
                "words": [
                    {
                        "style": {"bold": True},
                        "value": "This vulnerability is present in your install specifier range.",
                    },
    ...
    content = style_lines(
        to_print, columns - 3, styled_text, start_line="", end_line=""
    )
...
def build_other_options_msg(
    ...
    if secure_options:
        if len(secure_options) == 1:
            raw_pre_other_options += (
                f"version without known vulnerabilities {outside}is"
            )
        else:
            raw_pre_other_options += (
                f"versions without known vulnerabilities {outside}are:"
            )

        other_options_msg = (
            f"{raw_pre_other_options} {', '.join(secure_options)}".capitalize()
        )
...
def build_remediation_section(
    ...
    for pkg in remediations.keys():
        for req, rem in remediations[pkg].items():
            ...
            if fix_version:
                ...
                raw_recommendation = (
                    f"We recommend updating to version {fix_version} of {pkg}."
                )
                ...
                no_known_fix_msg = (
                    f"There is no known fix for {pronoun_word} {vuln_word}."
                )
    ...
    title = format_long_text(
        click.style("REMEDIATIONS", fg="green", bold=True), **kwargs
    )
...
def get_final_brief(
    ...
    remediation_text = (
        f" {rem_count} remediation{' was' if rem_count == 1 else 's were'} recommended."
        if is_using_api_key()
        else ""
    )
...
def get_final_brief_license(
    licenses: Iterable[str], kwargs: Optional[Dict[str, Any]] = None
) -> str:
    """
...
def get_report_brief_info(
    as_dict: Literal[True], report_type: int = 1, **kwargs: Any
) -> dict[str, Any]: ...
@overload
def get_report_brief_info(
    as_dict: Literal[False] = ..., report_type: int = 1, **kwargs: Any
...
def get_report_brief_info(as_dict: bool = False, report_type: int = 1, **kwargs: Any):
    ...
    brief_info = (
        [
            [
                {"style": False, "value": "Safety "},
                {"style": True, "value": "v" + (get_version() or "unknown")},
                {"style": False, "value": " is scanning for "},
                {
                    "style": True,
...
def build_primary_announcement(
    primary_announcement, columns: int, only_text: bool = False
) -> str:
    """
