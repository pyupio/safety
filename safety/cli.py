...
def generate_policy_file(name, path):
    ...
    try:
        ...
        click.secho(
            f"Unable to generate the policy file: {exc}",
            fg="red",
            file=sys.stderr,
        )
        sys.exit(EXIT_CODE_FAILURE)
...
def configure(
    ...
    if save_to_system:
        if not CONFIG_FILE_SYSTEM:
            click.secho(
                "Unable to determine the system-wide config path. "
                "Set the SAFETY_SYSTEM_CONFIG_PATH environment variable to specify the configuration file location.",
                fg="red",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    try:
        ...
        if (
            isinstance(e, OSError) and e.errno == 2 or isinstance(e, PermissionError)
        ) and save_to_system:
            click.secho(
                "Unable to save the configuration: writing to the system-wide Safety configuration file "
                "requires administrator privileges. Try running with 'sudo' or use '--save-to-user' instead.",
                fg="red",
                file=sys.stderr,
            )
        else:
            click.secho(
                f"Unable to save the configuration file: {e}. "
...
def check_updates(
    ...
    if not authenticated:
        if console.quiet:
            ...
            response = {
                "status": 401,
                "message": "Authentication failed. Please authenticate Safety and try again.",
                "data": {},
            }
            ...
            console.print()
            console.print(
                "[red]Safety is not authenticated. Please authenticate and try again.[/red]"
            )
            console.print()
            console.print(
                "To authenticate, run: [bold]safety auth login[/bold]\n"
                "For more information, run: [bold]safety auth --help[/bold]"
            )
        sys.exit(1)

    if not data:
        raise SafetyException(
            "No data found. Unable to retrieve update information from the Safety server. "
            "Check your network connection and authentication status."
        )
