...
from typing import TYPE_CHECKING, Any, Dict, Generator, Iterator, List, Optional, Tuple, Union, cast, Literal
...
from safety.scan.command import ScannableEcosystems, initialize_file_finder, scan_project_directory
from safety.scan.decorators import inject_metadata
from safety.scan.finder import FileFinder
from safety.scan.main import download_policy, load_policy_file, process_files, resolve_policy
from safety_schemas.models import (
    Ecosystem, FileModel, FileType, RemediationModel, ReportModel, 
    ReportSchemaVersion, ScanType, VulnerabilitySeverityLabels, 
    Vulnerability, MetadataModel, TelemetryModel, ProjectModel, Stage, AuthenticationType
)
...
ScanResult = Union[InitScanResult, ProgressScanResult, StatusScanResult, UploadingScanResult, CompleteScanResult]

LOG = logging.getLogger(__name__)

if TYPE_CHECKING:
    from safety_schemas.models import ConfigModel, ProjectModel, MetadataModel, \
    TelemetryModel, ReportModel, FileModel
...
def init_scan(
    ...
    yield InitScanResult(
        type=ScanResultType.INIT,
        dependencies=count
    )
    ...
    for idx, (path, analyzed_file) in enumerate(process_files(
        paths=file_paths, 
        config=config, 
        ...
        for spec in affected_specifications:
            ...
            for vuln in spec.vulnerabilities:
                ...
                severity = None
                if hasattr(vuln, 'CVE') and vuln.CVE and hasattr(vuln.CVE, 'cvssv3') and vuln.CVE.cvssv3:
            ...
            if spec.remediation:
                ...
                if has_recommended_version:
                    current_fixes += 1
                    current_resolved_vulns += len([v for v in spec.vulnerabilities if not v.ignored])
    ...
    yield UploadingScanResult(
        ...
        message="Preparing to upload scan results"
    )
...
def start_scan(
    ...
    use_server_matching: bool = False) -> Generator['ScanResult', None, Tuple['ReportModel', List['FileModel']]]:
    """
    ...
    command_name = "scan"
    telemetry = build_telemetry_data(telemetry=telemetry_enabled,
