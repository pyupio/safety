...
class TestUtils(unittest.TestCase):
    def setUp(self):
        ...
        self.vulns = list(
            map(
                lambda cve_kwargs: get_vulnerability(cve_kwargs=cve_kwargs).to_dict(),
                severity_kwargs,
            )
        )
    ...
    def test_highest_base_score(self):
        cvssv3_vulns = list(
            filter(
                lambda v: (v.get("severity", {}) or {}).get("cvssv3", {}), self.vulns
            )
        )
    ...
    def test_generate_issue_title_unpinned_remediation(self):
        ...
        self.assertEqual(
            issue_title, "Security Vulnerability in requests>=2.21.0,<2.22.0"
        )
    ...
    def test_generate_title_pinned_remediation(self):
        ...
        self.assertEqual(
            title, "Update requests from 2.21.0 to 2.22.0 to fix 1 vulnerability"
        )

        title = utils.generate_title(self.pkg, self.pinned_remediation, self.vulns)
        self.assertEqual(
            title, "Update requests from 2.21.0 to 2.22.0 to fix 4 vulnerabilities"
        )

    def test_generate_title_unpinned_remediation(self):
        title = utils.generate_title(
            self.pkg, self.unpinned_remediation, [self.vulns[0]]
        )
        self.assertEqual(
            title,
            "Update requests from >=2.21.0,<2.22.0 to 2.22.0 to fix 1 vulnerability",
        )

        title = utils.generate_title(self.pkg, self.unpinned_remediation, self.vulns)
        self.assertEqual(
            title,
            "Update requests from >=2.21.0,<2.22.0 to 2.22.0 to fix 4 vulnerabilities",
        )
