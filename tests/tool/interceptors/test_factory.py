...
class TestFactory(unittest.TestCase):
    ...
    def test_auto_select_unix_like(self):
        ...
        for platform in unix_platforms:
            with self.subTest(platform=platform):
                with patch("safety.tool.interceptors.factory.platform", platform):
                    interceptor = create_interceptor()
                    self.assertIsInstance(interceptor, UnixAliasInterceptor)
    ...
    def test_unsupported_platform(self):
        with self.assertRaises(NotImplementedError) as context:
            create_interceptor()
        self.assertIn(
            "Platform 'unsupported_os' is not supported", str(context.exception)
        )

    def test_invalid_interceptor_type(self):
        invalid_type = "INVALID_TYPE"
        with self.assertRaises(KeyError):
            create_interceptor(invalid_type)  # type: ignore[arg-type]
