...
def generate_report(paths, obj):
    """
    Processes the files and sends them via a POST request if server matching is enabled.

    Args:
        paths: A dictionary of file paths by file type.
        config: The configuration model (optional).
        use_server_matching: Whether to use server-side matching.
        obj: The object containing authentication details.
        target: The target path for metadata generation.
    """

    files = []
    meta = build_meta(Path("."))
    for file_type_key, f_paths in paths.items():
        file_type = FileType(file_type_key)
        if not file_type or not file_type.ecosystem:
            continue
